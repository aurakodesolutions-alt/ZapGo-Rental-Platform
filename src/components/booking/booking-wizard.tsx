...
import { Step5_Review } from "./step-5-review"; // ⬅ NEW
...
const steps = [
    ...
    { id: 5, component: Step5_Review }, // ⬅ swapped in
];
...
export function BookWizard() {
    ...
    useEffect(() => {
        ...
        if (vehicleParam) initialDraft.vehicle = { id: vehicleParam };
        if (planParam) initialDraft.plan = planParam.charAt(0).toUpperCase() + planParam.slice(1);

        if (Object.keys(initialDraft).length > 0) setDraft(initialDraft);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    const goToNextStep = () => setCurrentStep((p) => Math.min(p + 1, steps.length + 1));
    const goToPrevStep = () => setCurrentStep((p) => Math.max(p - 1, 1));

    if (draft.bookingId && draft.bookingCode) return <BookingSuccess />;
    ...
    return (
        <div className="flex flex-col min-h-dvh! bg-muted/30">
            ...
            <WizardFooter currentStep={currentStep} onNext={goToNextStep} onBack={goToPrevStep} />
        </div>
    );
}
